...
def preprocess_dataframe(
    ...
    processed["token_ids"] = processed["token_ids"].map(lambda ids: ids[:max_len])
    ...
    processed = processed[processed["token_ids"].map(len) > 0].reset_index(drop=True)

    processed["length"] = processed["token_ids"].map(len)
    processed["padded_ids"] = processed["token_ids"].map(
    lambda ids: pad_sequence(ids, max_len=max_len, pad_idx=pad_idx)
    )
